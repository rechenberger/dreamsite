const plugin = (schema, documents, config) => {
  ...
  const genericTypesAndFunctions = `
  ...
  documents.forEach((doc) => {
    doc.document.definitions.forEach((definition) => {
      ...
      if (isQuery || isMutation) {
        ...
        operationFuncDefinitions = [...operationFuncDefinitions, text]
      }
    })
  })

  // Only output if operations in file
  if (!operationFuncDefinitions.length) {
    return ''
  }

  // CREATE TEXT TO APPEND
  const overallText = [
    genericTypesAndFunctions,
    ...operationFuncDefinitions,
  ].join('\n')
  return overallText
}
